...
def CTD(seq):
	...
	for i in range(len(seq)-1):
		...
		if seq[i]=="C":
			...
			if c == num_C:
				C4_dis=((i*1.0)+1)/n
	return(float(num_A/n),float(num_T/n),float(num_G/n),float(num_C/n),float(AT_trans/(n-1)),float(AG_trans/(n-1)),float(AC_trans/(n-1)),float(TG_trans/(n-1)),float(TC_trans/(n-1)),float(GC_trans/(n-1)),float(A0_dis),float(A1_dis),float(A2_dis),float(A3_dis),float(A4_dis),float(T0_dis),float(T1_dis),float(T2_dis),float(T3_dis),float(T4_dis),float(G0_dis),float(G1_dis),float(G2_dis),float(G3_dis),float(G4_dis),float(C0_dis),float(C1_dis),float(C2_dis),float(C3_dis),float(C4_dis))
